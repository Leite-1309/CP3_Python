...
# Lê os arquivos TXT e JSON e junta todos os alunos em uma lista única.
# Assim, as outras funções não precisam abrir os arquivos de novo.
def ler_txt_json():
    alunos = []

    # Abre o TXT em modo leitura.
    # O encoding="utf-8" ajuda a manter nomes com acento funcionando.
    with open("entrada_alunos.txt", "r", encoding="utf-8") as arquivo_txt:
        for linha in arquivo_txt:
            ...
            # Exemplo: 001 - João Silva - ADS - Nota: 8.5
            partes = linha.split("-")
    ...
    with open("entrada_alunos.json", "r", encoding="utf-8") as arquivo_json:
        ...
        for aluno in alunos_json:
            # No JSON o id vem como número.
            # Aqui ele vira texto com três dígitos, tipo 006, 007, 008.
            aluno_formatado = {
                "id": f"{aluno['id']:03d}",
                "nome": aluno["nome"],
                "curso": aluno["curso"],
